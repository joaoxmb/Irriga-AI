...
async function definirParametros() {
  ...
  await aiRequest(prompt)
    .then(async (data) => {
      ...
      await chat("ia", { text: `Com base na necessidade hidríca da ${PARAMS.planta.nome}, cheguei à conclusão que a umidade do solo ideal é de ${PARAMS.umidade.min}% a ${PARAMS.umidade.max}%`, delay: 2000 })
      await chat("ia", { text: `Aqui está os dias da semana que será feito a rega` })
      await chat("ia", {
        text: `${PARAMS.semana
          .reduce((p, c, index) => {
            const week = ["Domingo", "Segunda-Feira", "Terça-Feira", "Quarta-Feira", "Quinta-Feira", "Sexta-Feira", "Sabádo"];
    ...
    .catch(async (error) => {
      ...
      await chat("ia", { text: "Desculpe, terei que pedir para que você preencha novamente o campo de qual planta estaremos regando.", delay: 1000 })
...
async function perguntarNome() {
  ...
  await chat("ia", { text: "Esse é um passo fundamental para que eu possa definir os paramêtros ideias para ela.", delay: 1000 })
...
async function perguntarCidade() {
  await chat("ia", { text: `É um prazer conhecê-lo ${SYSTEM.userName} 😉`, delay: 1000 })
  ...
  await chat("ia", { text: `Que legal, acho linda a cidade de ${SYSTEM.city} 🌇` })
}
async function perguntarPlanta() {
  await chat("user", { text: "Nome da planta ou plantas."}, (plant) => {SYSTEM.plant = plant;})
  await chat("ia", { text: `Estou pesquisando sobre a ${SYSTEM.plant} para definir os parâmetros ideias para ela.`, delay: 1000 })
...
async function inserirNoSistema() {
  ...
  .then(async () => {
    await chat("ia", { text: "Paramêtros enviado com sucesso 🥳" });
  ...
  .catch(async () => {
    ...
    await chat("ia", { text: "Estarei reenviando os parâmetros!", delay: 2000 });
...
export default async function talk() {
  ...
  await perguntarPlanta()
  await definirParametros()
  await chat("ia", { text: `${SYSTEM.userName.split(" ")[0]} agora eu estarei enviando todos os paramêtros que defini para o nosso sitema.`, delay: 2000 })
  await inserirNoSistema()
  await despedida()
}